...
class BaseHandler(tornado.web.RequestHandler):
    ...
    @gen.coroutine
    def excutesql(self,sqlitem,sqlparam=None):
        sqldict={
...
class HomeHandler(BaseHandler):
    ...
    def get(self):
        ...
        if self.current_user == None:
            ...
            newslist,newslist_finance,newslist_tech,newslist_ent,stock_report,business_newscount,business_count,business_list,business_public_list,vippersonal_list,stockpersonallist,fundpersonallist,banklist,stockcompanylist,fundcompanylist,vipfundpersonal_list  =yield [
              #newslist
              self.excutesql('sql_newslist'),
              # newslist_finance
              self.excutesql('sql_newslist_finance'),
              #newslist_tech
              self.excutesql('sql_newslist_tech'),
              #newslist_ent
              self.excutesql('sql_newslist_ent'),
              #stock_report
              self.excutesql('sql_stock_report'),
              #business_newscount(get)
              self.excutesql_get('sql_business_newscount'),
              #business_count(get)
              self.excutesql_get('sql_business_count'),
              #business_list
              self.excutesql('sql_business_list'),
              #business_public_list
              self.excutesql('sql_business_public_list'),
              #vippersonal_list
              self.excutesql('sql_vippersonal')
                #stockpersonallist
                ,self.excutesql('sql_personal')
                #fundpersonallist
                ,self.excutesql('sql_fundpersonal')
                #banklist
                ,self.excutesql('sql_banklist')
                #stockcompanylist
                ,self.excutesql('sql_stockcompanylist')
                #fundcompanylist
