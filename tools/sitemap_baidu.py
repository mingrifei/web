...
my_file = Path("sitemap_baidu")
if my_file.is_file():
    rm = os.remove('sitemap_baidu') #删除文件。
dir = os.popen('mkdir sitemap_baidu') #自动新建一个存放sitemap.xml的文件夹，默认叫sitemap，可自行修改。
path = 'sitemap_baidu/'#设定sitemap.xml文件存放的路径，别忘记了结尾的斜杠！
path1 = 'static/sitemap_baidu/'#设定sitemap.xml文件存放的路径，别忘记了结尾的斜杠！
...
my_file = Path("sitemap_baidu")
if my_file.is_file():
    rm = os.remove('../statics/sitemap_baidu') #删除文件。
cp = os.popen('\cp -rf sitemap_baidu ../statics/') #自动新建一个存放sitemap.xml的文件夹，默认叫sitemap，可自行修改。
