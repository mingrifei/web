...
class BaseHandler(tornado.web.RequestHandler):
    ...
    async def orm_query_all(self,sql,args=None):
        starttime = datetime.datetime.now()
        rs=await self.application.dbs_query(sql,args)
        endtime = datetime.datetime.now()
        #print(sql,'执行情况为', (endtime - starttime))
        return rs
    ...
    async def vexcutesql(self,sqlitem,sqlparam=None):
        starttime = datetime.datetime.now()
        sqldict={
            "sql_vippersonal": "SELECT RPI_PHOTO_PATH,AOI_ID, RPI_NAME, SCO_NAME, AOI_NAME, MD5(AOI_NAME) VAOI_NAME, ECO_NAME, PTI_NAME, CER_NUM, MD5(CER_NUM) AS vcer_num FROM `bigdata`.`person_stock_info` LIMIT 4",
            "sql_personal": "SELECT AOI_ID, RPI_NAME, SCO_NAME, AOI_NAME, MD5(AOI_NAME) VAOI_NAME, ECO_NAME, PTI_NAME, CER_NUM, MD5(CER_NUM) AS vcer_num FROM  `bigdata`.`person_stock_info` LIMIT 10",
            "sql_fundpersonal": "SELECT AOI_ID, RPI_NAME, SCO_NAME,MD5(RPI_ID) RPI_ID, AOI_NAME, MD5(AOI_NAME) VAOI_NAME, ECO_NAME, PTI_NAME, CER_NUM, MD5(CER_NUM) AS VCER_NUM  FROM `bigdata`.`person_fund_base_info`  LIMIT 11",
            ...
            "sql_newslist": "SELECT `t_news`.`id`,     `t_news`.`title`,     `t_news`.`pubtime`,     `t_news`.`url`,     `t_news`.`tag`,     `t_news`.`refer`,     `t_news`.`body`,     `t_news`.`link_business_id` FROM `bigdata`.`t_news` order by  pub_time desc limit 6 ",
            "sql_newslist_finance": "SELECT `t_news`.`id`,     `t_news`.`title`,     `t_news`.`pubtime`,     `t_news`.`url`,     `t_news`.`tag`,     `t_news`.`refer`,     `t_news`.`body`,     `t_news`.`link_business_id`,`t_news`.`stkname` FROM `bigdata`.`t_news` where length(stkcode)>1 order by  pub_time desc limit 10 ",
            ...
            "sql_business_public_list": "SELECT t1.business_id, t1.business_name, t1.business_industry, t1.business_score FROM ( SELECT * FROM `bigdata`.`business_base` WHERE business_plate = 'A' ) AS t1  LIMIT 10",
            "sql_vipfundpersonal":"SELECT RPI_PHOTO_PATH AS RPI_PHOTO_PATH,MD5(AOI_ID) AOI_ID,MD5(RPI_ID) RPI_ID, RPI_NAME, SCO_NAME, AOI_NAME AOI_NAME, MD5(AOI_NAME) VAOI_NAME, ECO_NAME, PTI_NAME, CER_NUM, MD5(CER_NUM) AS VCER_NUM FROM   `bigdata`.`person_fund_base_info`  LIMIT 4",
        ...
        if sqlparam is not None:
            v_result=await self.orm_query_all(sqldict[sqlitem],sqlparam)
        else:
            v_result=await self.orm_query_all(sqldict[sqlitem])

        return v_result

    async def vexcutesql_get(self,sqlitem,sqlparam=None):
        sqldict={
        ...
        if sqlparam is not None:
            v_result=await self.orm_query_get(sqldict[sqlitem],sqlparam)
        else:
            v_result=await self.orm_query_get(sqldict[sqlitem])
        return Row(v_result)
class HomeHandler(BaseHandler):
    async def get(self):
        ...
        if self.current_user == None:
            ...
            newslist = await self.vexcutesql('sql_newslist')
            newslist_finance = await self.vexcutesql('sql_newslist_finance')
            newslist_tech = await self.vexcutesql('sql_newslist_tech')
            newslist_ent = await self.vexcutesql('sql_newslist_ent')
            stock_report = await self.vexcutesql('sql_stock_report')
            business_newscount = await self.vexcutesql_get('sql_business_newscount')
            business_count = await self.vexcutesql_get('sql_business_count')
            business_list = await self.vexcutesql('sql_business_list')
            business_public_list = await self.vexcutesql('sql_business_public_list')
            vippersonal_list = await self.vexcutesql('sql_vippersonal')
            stockpersonallist = await self.vexcutesql('sql_personal')
            fundpersonallist = await self.vexcutesql('sql_fundpersonal')
            banklist = await self.vexcutesql('sql_banklist')
            stockcompanylist = await self.vexcutesql('sql_stockcompanylist')
            fundcompanylist = await self.vexcutesql('sql_fundcompanylist')
            vipfundpersonal_list = await self.vexcutesql('sql_vipfundpersonal')
    ...
    async def get(self):
        ...
        if business_detail_base is not None:
            ...
            self.render("business_detail.html", userinfo=self.current_user, business_detail_base=business_detail_base,
                        ...
                        business_detail_changes=business_detail_changes,reportlists=reportlist,newslists=newslist,sbzfgjjlist=sbzfgjjlist,menlists=menlist)
    ...
    async def get(self):
        ...
        if news_type == 'all':
            if vreport_search != '':
                ...
                reportlist=await self.vexcutesql('sql_report_search',sqlparam=(report_search, report_search, report_search, report_search,news_page))
            else:
                reportlist=await self.vexcutesql('sql_report_search_noparam',(0))
        else:
            reportlist=await self.vexcutesql('sql_report_search_noparam1',(news_type,0))
        self.render("stock_report_list.html", userinfo=self.current_user, reportlists=reportlist, page_count=page_count,news_page=int(vnews_page), report_search=vreport_search)
...
class jobs_detail_Handler(BaseHandler):
    ...
    async def get(self):
        id = self.get_argument("id", None)
        job_detail=await self.vexcutesql_get('sql_jobs_detail',(id))
...
class jobs_list_Handler(BaseHandler):
    ...
    async def get(self):
        ...
        if jobs_search != '':
            ...
            jobslist =await self.vexcutesql('sql_jobs_list_search',sqlparam=(jobs_search,news_page))
            jobslistcount =await self.vexcutesql_get('sql_jobs_list_search_count',sqlparam=(jobs_search))
        else:
            jobslist = self.vexcutesql('sql_jobs_list_noparam',sqlparam=(news_page))
            jobslistcount =await self.vexcutesql_get('sql_jobs_list_noparam_count')
...
class AuthLoginHandler(BaseHandler):
    ...
    async def get(self):
        ...
        if not author:
            ...
            return
        hashed_password = executor.submit(
