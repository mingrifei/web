...
class HomeHandler(BaseHandler):
    def get(self):
        ...
        if self.current_user==None:
            ...
            newslist=self.db.query("SELECT `t_news`.`id`,     `t_news`.`title`,     `t_news`.`pubtime`,     `t_news`.`url`,     `t_news`.`tag`,     `t_news`.`refer`,     `t_news`.`body`,     `t_news`.`link_business_id` FROM `bigdata`.`t_news` order by  str_to_date(pubtime, '%%Y-%%m-%%d') desc limit 6 ")
            newslist_finance=self.db.query("SELECT `t_news`.`id`,     `t_news`.`title`,     `t_news`.`pubtime`,     `t_news`.`url`,     `t_news`.`tag`,     `t_news`.`refer`,     `t_news`.`body`,     `t_news`.`link_business_id` FROM `bigdata`.`t_news` where tag='finance' order by  str_to_date(pubtime, '%%Y-%%m-%%d') desc limit 10 ")
            newslist_tech=self.db.query("SELECT `t_news`.`id`,     `t_news`.`title`,     `t_news`.`pubtime`,     `t_news`.`url`,     `t_news`.`tag`,     `t_news`.`refer`,     `t_news`.`body`,     `t_news`.`link_business_id` FROM `bigdata`.`t_news` where tag='tech' order by  str_to_date(pubtime, '%%Y-%%m-%%d') desc limit 10 ")
            newslist_ent=self.db.query("SELECT `t_news`.`id`,     `t_news`.`title`,     `t_news`.`pubtime`,     `t_news`.`url`,     `t_news`.`tag`,     `t_news`.`refer`,     `t_news`.`body`,     `t_news`.`link_business_id` FROM `bigdata`.`t_news` where tag='ent' order by  str_to_date(pubtime, '%%Y-%%m-%%d') desc limit 10 ")
    ...
    def get(self):
        ...
        if news_type == 'all':
            if vreport_search != '':
                #report_search = '%' + vreport_search + '%'
                report_search = vreport_search
                sql = "SELECT `t_news`.`id`,`t_news`.`title`,`t_news`.`pubtime`,`t_news`.`url`,`t_news`.`tag`,`t_news`.`refer`,`t_news`.`abstract`,`t_news`.`body`,`t_news`.`link_business_id`,`t_news`.`newsid`,`t_news`.`stkcode`,`t_news`.`stkname`,`t_news`.`stkindustry`,`t_news`.`instime` FROM `bigdata`.`t_news` where  length(stkcode)>1 and (stkname = %s or stkcode = %s or stkindustry = %s or pubtime = %s)  order by str_to_date(pubtime, '%%Y-%%m-%%d') desc limit {news_page}, 100".format(
                    news_page=news_page)
                newslist = self.db.query(sql,report_search,report_search,report_search,report_search)
            else:
                sql = "SELECT `t_news`.`id`,`t_news`.`title`,`t_news`.`pubtime`,`t_news`.`url`,`t_news`.`tag`,`t_news`.`refer`,`t_news`.`abstract`,`t_news`.`body`,`t_news`.`link_business_id`,`t_news`.`newsid`,`t_news`.`stkcode`,`t_news`.`stkname`,`t_news`.`stkindustry`,`t_news`.`instime` FROM `bigdata`.`t_news` where  length(stkcode)>1 order by str_to_date(pubtime, '%%Y-%%m-%%d') desc limit {}, 100".format(
                    news_page)
                newslist = self.db.query(sql)
        else:
            sql = "SELECT `t_news`.`id`,`t_news`.`title`,`t_news`.`pubtime`,`t_news`.`url`,`t_news`.`tag`,`t_news`.`refer`,`t_news`.`abstract`,`t_news`.`body`,`t_news`.`link_business_id`,`t_news`.`newsid`,`t_news`.`stkcode`,`t_news`.`stkname`,`t_news`.`stkindustry`,`t_news`.`instime` FROM `bigdata`.`t_news` where  length(stkcode)>1 and tag=%s order by str_to_date(pubtime, '%%Y-%%m-%%d') desc limit {}, 100".format(
                news_page)
            newslist = self.db.query(sql, news_type)
